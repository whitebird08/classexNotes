essentials.firstElementChild.firstElementChild.addEventListener('click', function(){
  document.getElementsByTagName('img')[0].src = './images/milk.jpeg'})

////////
// higher order break out
// "pipelining"
...
foods.filter(function(food){
   return department == grocery || == meat
...
function department (food){
  return food.department === "grocery" || food.dpeartment === 'meat';
...
people = [{firstName: "Hyman", lastName: "Runte", country: "United States", age: 23}];

lastName && beginsWith"L" => 

format country 
if (age > 60) output this.country

{"United States": X, "Canada": Y, etc...}
///
function lastStartsWith(people){
  return people.lastName forEach.charAt[0] == "L"
}
///
function formatCountry(person){
  if (people.country = "UK" || "England"){
    return "UK"
  }  if {
    (people.country = "US" || "United States"){
    return "US"
  } ]if {
    (people.country = "IRL" || "Ireland"){
    return "IRL"
  } if {
    (people.country = "CAN" || "Canada"){
    return "CAN"
  }   
  return person
} .map(function(person){
  return person.country
  .reduce(function(countobj, country){
    if(countobj[country]) !== undefined)

  })
})
///
function overSixty(people){
  if (age > 60) return people.country
}
///
function peoplePerCountry(people){
  var totalUS = [];
  var totalUK = [];
  var totalIRL = [];
  var totalCAN = [];
...
myArray.reduce(function ( storedBoolean, listOfAnyType ) {

Brett Lyons 
return ( listOfAnyType == 3 ) && storedBoolean;
